...
async function run() {
  try {
    ...
    const scan = (core.getInput('scan', { required: false }) || './').split(',').map(dir => `${path.resolve(path.join('..', dir.trim()))}/**/package.json`)
    ...
    if (ignoreList.length > 0) {
      (await globby(
        ignoreList
          ...
          .map(dir => `${path.resolve(dir.trim()).trim()}/**/package.json`)
      ))
    ...
    // Scan for modules
    core.debug('DEBUG GLOB PATH: ' + (await globby(scan.concat(['!**/node_modules']))).join('\n'))
    const paths = new Set(await globby(scan.concat(['!**/node_modules'])))

    // Remove ignored directories
    if (ignore.size > 0) {
      console.log(`Directories to ignore:\n\t- ${ignoreList.join('\n\t- ')}`)
