...
async function run() {
  try {
    ...
    const ignore = core.getInput('ignore', { required: false }) ? new Set(await globby((core.getInput('ignore', { required: false }) || "").split(',')).map(dir => `${path.resolve(dir.trim())}/**/package.json`)))) : new Set()
