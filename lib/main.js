...
async function run() {
  try {
    ...
    const scan = (core.getInput('scan', { required: false }) || './').split(',').map(dir => `${path.resolve(path.join('..', dir))}/**/package.json`)
    const ignore = core.getInput('ignore', { required: false }).trim().length > 0 ? new Set(await globby((core.getInput('ignore', { required: false }) || "").split(',')).map(dir => `${path.resolve(dir.trim())}/**/package.json`)))) : new Set()
    const npm = new NpmRegistry(token, core.getInput('registry', { required: false }))
console.log('IGNORE', ignore)
core.setFailed('Intentional')
return
    console.log(scan)
...
console.log(scan.concat(['!**/node_modules']))
console.log(paths)
...
console.log(paths)
    ...
    paths.forEach(file => {
      ...
      core.debug(`Attempting to publish from "${file}"`)
